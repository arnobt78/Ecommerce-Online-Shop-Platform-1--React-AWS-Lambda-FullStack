...
## Status (2026-08-02)
C1 Phases 1–9 done (REQ-1200…1673), pushed to `origin/main` through `4c39bed`; REQ-1673 uncommitted, pending user go-ahead. Catalog: 17 books. Phase 8: payment-tampering fix, auto-refund-on-cancel, invoice download, order idempotency, per-entity admin analytics, deterministic restock/fraud/pricing/churn signals, on-demand AI review-sentiment, admin low-stock digest, `/email/send` open-relay fix. Phase 9: wishlist, back-in-stock alerts, coupons, guest checkout, live Shippo webhook, scheduled digest+AI-summary jobs, CSV import/export, return/RMA, refresh tokens, AI description generator, fuzzy search, AI ticket-reply drafts.

Seven post-implementation passes, each verified live against the real dev DB not just compiled: REQ-1668 (3 minor hardening gaps), REQ-1669 (whole-codebase, zero further gaps), REQ-1670 (demo coupons + a whole-page-spinner UX fix), REQ-1671 (guest returns gap; **also found and fixed a severe pre-existing bug**: `wishlist.routes.ts`'s blanket `router.use(requireAuth)`, mounted with no path prefix, was silently 401-blocking every route mounted after it — stock-alerts, coupon validation, the real Shippo webhook), REQ-1672 (shared `Order` type was missing 4 real fields, which had hidden a guest-return stale-input edge case), REQ-1673 (pattern-matched the same guest-parity gap onto `GET /orders/:id/invoice` — guests could never re-download their invoice, only auth-only like returns had been; same `optionalAuth`+email fix, added to `GuestOrderLookupPage`). Coolify/VPS deferred. Live demo still legacy AWS. Gate 2 / Red Team optional. Lint+tsc+build clean FE+BE, `npm audit`/`depcheck` clean (backend 0 vulns; frontend's one high advisory is the known RSC-mode-only react-router CVE, inapplicable to this SPA, already pinned per Gotchas).
