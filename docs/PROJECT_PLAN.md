...
### Phase 9 — New commerce/admin/AI features + two pre-commit audits (REQ-1656 to REQ-1669) — added 2026-08-02, implemented 2026-08-02

User-selected subset of a post-Phase-8 feature brainstorm. Declined and not built: Stripe saved payment methods (user's own call — building card-on-file trust/PCI-adjacent infra is disproportionate for this project), admin 2FA (this is a demo/showcase app — added login friction for reviewers outweighs the benefit here).

1. ✅ Wishlist / favorites (REQ-1656). New `Wishlist` join table (unique `userId`+`productId`, idempotent add). Heart-toggle on `ProductCard`/`ProductDetail` (new `useWishlistedProductIds()` derives an O(1) lookup Set from one cached query, zero per-card fetches); new "My Wishlist" section on the Dashboard.
2. ✅ Back-in-stock email alert (REQ-1657). New `StockAlert` table. Fires on a true 0→positive transition in **both** admin product edits and order-cancel/refund stock restoration. "Notify me" button on `ProductDetail`, instant for logged-in users, inline email capture for guests.
3. ✅ Coupon / discount codes (REQ-1658). New `Coupon` model. Discount recomputed server-side from the server's own subtotal at `create-intent` and re-derived from the verified PaymentIntent at order-creation (never a client-sent figure — same discipline as REQ-1637); usage count only increments on an actual new-order insert. `AdminCouponsPage` CRUD + Cart apply-coupon UI.
4. ✅ Guest checkout (REQ-1659) — additive, not a replacement. New `optionalAuth` middleware; a guest gets a synthetic `guest_<uuid>` identity so every existing `order.userId !== req.user.id` check across the codebase keeps working unmodified. No-auth `GET /orders/guest/:orderId?email=` lookup + `GuestOrderLookupPage`. **Found and fixed a real bug**: `/payment-success`/`/payment-cancel` were wrapped in `<ProtectedRoute>`, which would have redirected any guest to `/login` post-payment, losing the order — unwrapped both.
5. ✅ Live shipment tracking via Shippo webhook (REQ-1660). `POST /webhooks/shippo/:secret` — Shippo doesn't sign Track webhooks like Stripe does, so verification is a shared secret in the URL path itself, plus rate limiting. Advances `shipped` → `delivered` on a real `DELIVERED` event; manual override still available.
6. ✅ Real scheduler for the low-stock digest + weekly AI sales-summary email (REQ-1661). `node-cron`, opt-in via `SCHEDULE_JOBS_ENABLED` (default off — verified via a live boot log that local dev never fires a real email by accident). Daily digest logic shared with the REQ-1654 manual route via one `getStockDigestBreakdown()` helper. Weekly summary computes a lean server-side analytics summary via Prisma and feeds the existing REQ-1613 AI chain, degrading to numbers-only if the LLM is unavailable.
7. ✅ CSV import/export for products and orders (REQ-1662). New `csv-parse`/`csv-stringify` deps (RFC4180 correctness). Products: export + row-by-row import with per-row error reporting. Orders: **export only, deliberately no import** — orders are transactional records created only through the verified checkout flow; bulk-creating them via CSV would bypass those guarantees.
8. ✅ Customer-initiated return/RMA flow (REQ-1663). New `ReturnRequest` model, 30-day return window. Approve reuses the real `refundOrderPayment()` Stripe flow — **refactored out of `orders.routes.ts` into `orders.service.ts` and exported** so `returns.service.ts` could reuse it instead of a third parallel refund implementation. "Request Return" on customer `OrderDetailPage`; `AdminReturnsPage` for triage.
9. ✅ Refresh tokens (REQ-1667, added mid-round — flagged when the user's original suggestion list included it but it was accidentally dropped from this phase's initial scoping). Access token shortened 7d → 1h; new `RefreshToken` model (hash-only, rotated on every use — reuse of a revoked token is a theft signal); `POST /auth/refresh`/`POST /auth/logout`. Frontend `useTokenRefresh.ts` proactively renews on mount/focus/interval rather than retrofitting a 401-retry interceptor into ~15 raw-fetch service files. **Live-verified end-to-end** against the real dev DB: login → refresh (rotated) → old token correctly rejected on reuse → new access token authorized a real route → logout revoked it → reuse after logout correctly rejected.
10. ✅ AI product-description generator (REQ-1664). Reuses the multi-provider `lib/ai/` chain; draft only, never auto-published. **Found and fixed a real bug via live testing**: `long_description` was truncated at `max_tokens: 512` (same reasoning-token-budget issue previously hit and fixed for REQ-1613) — raised to 1024, re-verified with a complete real Gemini description.
11. ✅ Fuzzy product search (REQ-1665). `pg_trgm` enabled on the dev DB; `getAllProducts()` uses `similarity()`-ranked matching with a graceful fallback to exact `contains` if the extension isn't available. **Verified live**: the exact "atomic habbits" typo case named when this was scoped now correctly returns "Atomic Habits" (previously zero results).
12. ✅ AI-assisted ticket reply drafts (REQ-1666). Same on-demand admin-click pattern as REQ-1651/1664; draft only, never auto-sent, explicitly instructed not to promise outcomes only a human admin can commit to. **Verified live**: created a real ticket, generated a coherent context-specific draft, confirmed a non-admin gets 403, deleted the test ticket.
13. ✅ Pre-commit deep audit (REQ-1668). Full re-verification pass over all 12 items above before commit — traced every new file's wiring (route registration, `AllRoutes.tsx`, barrel exports, no orphans) and every new hook's `invalidateQueries` coverage end to end. Found and fixed 3 real minor hardening gaps: Shippo webhook secret compared with `!==` (timing side-channel) switched to `crypto.timingSafeEqual`; `POST /stock-alerts` (public, unauthenticated, email-collecting) had no rate limiter, added `publicWriteLimiter`; `src/services/index.ts` barrel was missing 5 of the newest services, completed for consistency. No functional/architectural defects found.
14. ✅ Second, whole-codebase audit (REQ-1669), after the above was committed and pushed. Widened scope beyond the Phase 9 diff to all 18 frontend hooks, all 18 backend routes, an orphaned-file scan across the entire repo, `depcheck`/`npm audit` both sides, and a full env-var/Vite-convention sweep. **Zero new gaps found** — the 3 fixes in item 13 were the complete set of real issues.

**Status: 100% done and verified, committed + pushed (`259d7ff`).** `tsc -b`/`eslint --max-warnings=0`/`vite build` (frontend) and `tsc --noEmit`/`eslint --max-warnings=0`/`tsc` build (backend) clean after every single item, not just at the end. Every risk-bearing item verified against real running state (a real Postgres dev DB, a real multi-provider LLM call, or both) — two real bugs found and fixed via that live testing (guest-checkout routing, AI description truncation — see items 4 and 10), plus 3 more minor hardening gaps found in the first pre-commit audit (item 13). A second, whole-codebase audit (item 14) after committing found zero further gaps. See `.agile-v/REQUIREMENTS.md` for full per-item evidence and `.agile-v/STATE.md` for the pass-by-pass trace.
