...
See `docs/PROJECT_PLAN.md` §6 for the authoritative, checkbox-tracked phase list and §11 for a running done/not-done summary. As of the latest pass, every phase through REQ-1669 is done and pushed to `origin/main` (Phase 9 added wishlist/favorites, back-in-stock alerts, coupon codes, guest checkout, a live Shippo tracking webhook, an opt-in scheduler for the low-stock digest + a new weekly AI sales-summary email, product CSV import/export, a customer return/RMA flow, refresh tokens, an AI product-description generator, `pg_trgm`-powered fuzzy product search, and AI-assisted support-ticket reply drafts, followed by two pre-commit audits: REQ-1668 closed 3 minor hardening gaps — timing-safe webhook secret comparison, a rate limit on the public stock-alert subscribe route, and a completed `services/index.ts` barrel — and REQ-1669, a second, whole-codebase pass covering every hook/route/env-var rather than just the Phase 9 diff, found zero further gaps); the only intentionally-deferred items are Coolify/VPS deployment (explicit user hold), wiring the customer address book's *default* address automatically into every checkout without an explicit selection step (optional/manual by design), Stripe saved payment methods and admin 2FA (both explicitly declined by the user this round), and semantic (meaning-based, not just typo-tolerant) search beyond `pg_trgm`.
