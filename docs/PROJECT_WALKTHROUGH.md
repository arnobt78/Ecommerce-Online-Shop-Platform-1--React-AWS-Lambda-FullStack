...
See `docs/PROJECT_PLAN.md` §6 for the authoritative, checkbox-tracked phase list and §11 for a running done/not-done summary. As of the latest pass, every phase through REQ-1655 is done (REQ-1655: post-implementation re-audit of Phase 8, which also closed a real pre-existing gap — `POST /email/send` now restricts non-admin callers' `to` to their own email or the fixed admin alert address, closing what had been an open relay); the only intentionally-deferred items are Coolify/VPS deployment (explicit user hold), wiring the customer address book's *default* address automatically into every checkout without an explicit selection step (optional/manual by design), and a scheduled cron trigger for the low-stock digest email (REQ-1654 ships the real digest itself, admin-triggered rather than automatic — adding a scheduler would be new backend infrastructure).
