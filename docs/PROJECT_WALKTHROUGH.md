...
See `docs/PROJECT_PLAN.md` §6 for the authoritative, checkbox-tracked phase list and §11 for a running done/not-done summary. As of the latest pass, every phase through REQ-1672 is implemented and verified (REQ-1656–1670 pushed to `origin/main`; REQ-1671/1672 implemented/verified, pending commit) — Phase 9 added wishlist/favorites, back-in-stock alerts, coupon codes, guest checkout, a live Shippo tracking webhook, an opt-in scheduler for the low-stock digest + a new weekly AI sales-summary email, product CSV import/export, a customer return/RMA flow, refresh tokens, an AI product-description generator, `pg_trgm`-powered fuzzy product search, and AI-assisted support-ticket reply drafts, followed by five polish passes: REQ-1668 closed 3 minor hardening gaps; REQ-1669, a whole-codebase pass, found zero further gaps; REQ-1670 seeded 2 demo coupons and fixed a loading-UX inconsistency; REQ-1671 gave guest checkout orders a way to request a return and, while live-testing that fix, found and fixed a severe pre-existing bug where an unscoped `router.use(requireAuth)` in `wishlist.routes.ts` was silently 401-blocking every route mounted after it in `app.ts` for unauthenticated callers — including the real Shippo webhook, which never sends a Bearer token (see §7); REQ-1672, re-auditing REQ-1671, found the shared `Order` type was missing 4 fields that actually exist on the Prisma model, which had let a guest-return edge case (verifying against a live editable input instead of the already-confirmed `order.guestEmail`) go unnoticed. The only intentionally-deferred items are Coolify/VPS deployment (explicit user hold), wiring the customer address book's *default* address automatically into every checkout without an explicit selection step (optional/manual by design), Stripe saved payment methods and admin 2FA (both explicitly declined by the user this round), and semantic (meaning-based, not just typo-tolerant) search beyond `pg_trgm`.
